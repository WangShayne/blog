...
# 96.循环赋值作用域问题
- 当为管道符时，管道符为非内建命令，会启动子shell进行循环，所以当前的shell则不能获取值
```
cat demo.txt|while read row
do
    a=${a}${row}
done
echo ${a} # 空
...
cat ./demo.txt|while read row
do
    exsitValue=`cat ./demo2.txt`
    echo ${exsitValue}${row} > ./demo2.txt
done
cat ./demo2.txt
```
...
ln 源文件 连接文件
# 硬连接
ln /root/demo /usr/bin/demo 
# 软连接
ln -s /root/demo /usr/bin/demo 
```
...
- 硬连接必须删除主体文件才会消失
- 软连接包含文件的路径，如果源文件移除或者移动则找不到主体文件，硬连接则无需担心此问题
- 软连接应用更加官方，可以跨越不通的文件系统、网络等进行链接。
...
- 执行多条命令
```
ssh -T root@ip "pwd; cat hello.txt"
```
...
# $0 bash
# $1 test
ssh root@ip 'bash -s' < test.sh test
```
